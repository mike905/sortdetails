    ...
    def print_aggregated_data(self, text_widget=None):
        ...
        if text_widget:
            for index, row in aggregated_data.iterrows():
                designation = row.get('Обозначение', 'Неизвестное обозначение')
                name = row.get('Наименование', 'Неизвестное наименование')
                total_qty = row.get('Общее количество', 'Неизвестное количество')
                note = row.get('Примечание', 'Нет примечания')
                line = f"{designation} - {name}: {total_qty} {note}\n"
    ...
    def choose_file(self):
        ...
        if file_path:
            try:
                ...
                # Добавим вывод столбцов перед их использованием
                print("Колонки агрегированных данных: ", aggregated_data.columns)
